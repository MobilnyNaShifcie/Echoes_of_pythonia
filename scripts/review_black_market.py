"""Reproducible market review; all evidence and isolated profiles stay in build/."""
import argparse
import os
...
def main():
    ...
    parser.add_argument('stage', choices=['before', 'after', 'chest-before', 'chest-after'])
    parser.add_argument('--tests', action='store_true')
    parser.add_argument('--only-tests', action='store_true')
    parser.add_argument('--test-script')
    args = parser.parse_args()
    work = ROOT / 'build/black-market-review' / args.stage
    ...
    commands = [] if args.only_tests else [
        ...
        ('capture', ['--rendering-method', 'gl_compatibility', '--rendering-driver', 'opengl3',
                     ...
                     '--script', 'res://tools/capture_black_market.gd', '--',
                     str(evidence), str(work / 'fixture_saves')])]
    ...
    for label, flags in commands:
        ...
        errors = [line for line in log.splitlines() if 'ERROR:' in line and
