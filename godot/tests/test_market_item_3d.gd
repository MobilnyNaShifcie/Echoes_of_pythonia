...
func test_rotation_goods_use_geometry_and_clear_without_orphans() -> void:
	...
	for item_id in ["common_essence", "spark_of_life", "grandmaster_elixir", "black_pearl", "hearth_core", "leviathan_scale", "azhar_sigil", "mastery_book", "mastery_attack_speed_book"]:
		view.build(item_id)
...
func test_reference_elixir_is_textured_volume_not_billboard() -> void:
	...
	assert_eq(body.mesh.surface_get_arrays(0)[Mesh.ARRAY_VERTEX], original_vertices, "Rotation never rebuilds a camera-facing plane")
...
func test_reference_pearl_has_solid_iridescent_body_and_separate_carved_gold() -> void:
	...
	for part in ["IridescentPearl", "EngravedGoldCradle", "LeftRearClaw", "LeftSweptClaw", "RightCrownClaw", "RightSweptClaw", "LowerGoldBead", "CarvedRimRelief", "LeftClawBevel"]:
		...
		assert_eq(arrays[Mesh.ARRAY_TEX_UV].size(), arrays[Mesh.ARRAY_VERTEX].size())
		assert_string_contains(node.material_override.albedo_texture.resource_path, "black_pearl.png")
	...
	assert_eq(pearl.mesh.surface_get_arrays(0)[Mesh.ARRAY_TEX_UV], uvs, "Artwork stays fixed on geometry during rotation")
...
func test_pearl_and_elixir_have_matching_reference_height() -> void:
	...
	assert_almost_eq(pearl.size.y / elixir.size.y, 1.0, 0.05, "Same reference height, with the pearl's own round proportions")
...
func test_manuscript_is_layered_volume_with_original_art_and_raised_fittings() -> void:
	...
	for part in ["EmbossedLeatherCover", "BackLeatherCover", "LayeredParchmentBlock", "RoundedLeatherSpine", "SilverCorner0", "SilverCorner1", "SilverCorner2", "SilverCorner3", "SwiftBladeRelief0", "SwiftBladeRelief1", "SwiftBladeRelief2", "LeatherClasp", "SilverBuckle", "ClaspDiamond", "CrimsonRibbon", "RaisedSpineBand0", "SpineSilverStud0", "ParchmentEdge00"]:
		var node: MeshInstance3D = view.model.get_node(part)
		assert_true(node.mesh is ArrayMesh, part + " is solid authored geometry")
		var arrays := node.mesh.surface_get_arrays(0)
		assert_eq(arrays[Mesh.ARRAY_TEX_UV].size(), arrays[Mesh.ARRAY_VERTEX].size())
		assert_gt(node.get_aabb().size.z, 0.001, part + " has depth")
		assert_string_contains(node.material_override.albedo_texture.resource_path,"mastery_attack_speed_book.png")
	...
	assert_eq(cover.mesh.surface_get_arrays(0)[Mesh.ARRAY_TEX_UV],uvs,"No camera-facing reconstruction on rotation")
...
func test_manuscript_lies_flat_on_the_counter_and_fits_viewport() -> void:
	...
	assert_lt(book.size.y/elixir.size.y,0.70,"A lying book is foreshortened, not standing upright like the bottle")
	assert_true(Rect2(Vector2.ZERO,view.size).encloses(book),"Ribbon and all corners remain in frame")
	var cover: MeshInstance3D = view.model.get_node("EmbossedLeatherCover")
	...
	assert_almost_eq(back.get_aabb().position.y,0.018,0.001,"The broad lower cover rests directly above the mat")
	...
	assert_lt(maximum.y-minimum.y,0.40,"The complete book lies down, not just its cover texture")
	...
	assert_gt(upper_normal.normalized().y,0.8,"The upper cover faces the camera/light instead of exposing its underside")
