	...
	for item_id in PARTS:
		...
		for part in PARTS[item_id]:
			...
			assert_eq(
				...
				"Solid surface, not a cutout plane"
			)
		...
		assert_gt(
			view.model.get_child_count(), 20, "Separate fittings instead of the former primitive"
		)
		var body: MeshInstance3D = view.model.get_node(PARTS[item_id][0])
		assert_gt(
			body.get_aabb().size.z, 0.25, "The principal body has a physical rear and thickness"
		)
...
func test_artifacts_lie_on_counter_and_remain_in_frame() -> void:
	...
	for item_id in PARTS:
		...
		assert_almost_eq(
			minimum_y, 0.018, 0.0001, item_id + " rests on its actual lowest solid point"
		)
		assert_almost_eq(view.model.get_node("ContactShadow").position.y, 0.004, 0.0001)
		var rect := projected_rect(view)
		assert_lt(
			rect.size.y / elixir.size.y,
			0.85,
			"A stable lying artifact is lower than the upright bottle"
		)
		assert_gt(rect.size.y / elixir.size.y, 0.2, "Original artifact proportions remain legible")
		assert_true(
			Rect2(Vector2.ZERO, view.size).encloses(rect),
			"Handle, charm and tip must not be cropped"
		)
...
func test_core_volume_tapers_at_silhouette_instead_of_extruding_texture_stripes() -> void:
	...
	for pixel: Vector2 in builder.body_outline:
		assert_almost_eq(
			builder._body_depth(pixel), 4.0, 0.01, "The two faces meet along a narrow stone edge"
		)
