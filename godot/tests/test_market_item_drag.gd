...
func test_drag_moves_same_live_world_and_cancel_restores_it() -> void:
	...
	assert_almost_eq(
		visual_size.x / slot.size.x, 1.5, 0.01, "Elixir is displayed 50 percent larger"
	)
	assert_true(
		slot._has_point(view.position + Vector2(view.size.x * 0.5, 20)),
		"The enlarged upper bottle remains grabbable"
	)
	...
	assert_eq(
		view.position,
		motion.position + slot._grab_offset,
		"Held model follows pointer at original grab point"
	)
...
func test_failed_purchase_returns_item_despite_accepted_drop() -> void:
	...
	var slot = _elixir_slot(market)
	slot._begin_drag_visual(Vector2(70, 35))
	market.inventory_drop_target._drop_data(
		Vector2.ZERO, {"kind": "black_market_offer", "offer_id": slot.offer_id}
	)
	slot.notification(Control.NOTIFICATION_DRAG_END)
...
func test_purchased_held_item_never_reappears_after_drag_end() -> void:
	...
	slot._begin_drag_visual(Vector2(70, 35))
	market.inventory_drop_target._drop_data(
		Vector2.ZERO, {"kind": "black_market_offer", "offer_id": offer}
	)
	slot.notification(Control.NOTIFICATION_DRAG_END)
...
func test_pearl_uses_elixir_scale_moves_as_live_model_and_returns_to_its_pad() -> void:
	...
	assert_true(
		slot._has_point(view.position + Vector2(dimensions.x * 0.3, dimensions.y * 0.3)),
		"The wide gold setting can be grabbed"
	)
	...
	assert_lt(
		view.size.distance_to(dimensions),
		0.001,
		"Same displayed size, allowing only subpixel global-transform rounding"
	)
	...
	assert_eq(
		market._session.player.inventory.count("black_pearl"),
		2,
		"Rotation still sells the original two-pearl lot"
	)
