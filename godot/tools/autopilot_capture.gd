...
func _run() -> void:
	...
	for resolution: Array in request.resolutions:
		...
		var hero_class := str(request.get("hero_class", ""))
		if not hero_class.is_empty() and not session.player.choose_class(hero_class):
			_fail("Invalid fixture hero class: " + hero_class)
			return
		session.hour = int(request.get("hour", 12))
					...
					. append(
						{
							...
							"background": combat.battlefield_texture.texture.resource_path,
							"hero": _actor_layout(combat.player_visual),
							"enemy": _actor_layout(combat.enemy_visual),
						}
					)
				)
	...
	manifest.store_string(
		JSON.stringify({"ok": not _failed, "images": _images, "layouts": _layouts}, "\t")
	)
	manifest.close()
...
func _show(app: Node, scenario: String) -> bool:
	if scenario.begins_with("combat_"):
		var enemy_id := scenario.trim_prefix("combat_")
		var region_id := Presentations.enemy_region_id(enemy_id, "")
		if region_id.is_empty() or Presentations.enemy_presentation(enemy_id).is_empty():
			return false
		# Resolve the same roster as production; never fall back to an unrelated map.
		app._current_session.current_location_id = region_id
...
func _actor_layout(actor) -> Dictionary:
	var sprite: TextureRect = actor.static_texture
	var texture: AtlasTexture = sprite.texture as AtlasTexture
	var rect := sprite.get_global_rect()
	var slot: Rect2 = actor.get_global_rect()
	return {
		"source": actor.source_texture().resource_path,
		"crop":
		[
			texture.region.position.x,
			texture.region.position.y,
			texture.region.size.x,
			texture.region.size.y
		],
		"painted_rect": [rect.position.x, rect.position.y, rect.size.x, rect.size.y],
		"slot_rect": [slot.position.x, slot.position.y, slot.size.x, slot.size.y],
