extends SceneTree
## Standalone test delivery; no app, save service or real player profile.

const OUTPUT := "res://../output/leviathan_scale/"
...
func capture(path: String) -> void:
	...
	var error := root.get_texture().get_image().save_png(OUTPUT + path + ".png")
	assert(error == OK, "Preview could not be written")
...
func render_preview() -> void:
	...
	var items := ["grandmaster_elixir", "black_pearl", "leviathan_scale", "azhar_sigil"]
	var prices := [7500, 7000, 13000, 12000]
	for i in items.size():
	...
	print("Scale drag returned the same live model to its mat")
	market.queue_free()
	await process_frame
	...
	original.texture = load("res://assets/items/materials/leviathan_scale.png")
	...
	export_root.name = "LeviathanScale"
	var document := GLTFDocument.new()
	var state := GLTFState.new()
	var result := document.append_from_scene(export_root, state)
	if result == OK:
		result = document.write_to_filesystem(state, OUTPUT + "leviathan_scale.glb")
	print("Scale GLB export: ", result, "; solid meshes: ", export_root.get_child_count())
	export_root.free()
