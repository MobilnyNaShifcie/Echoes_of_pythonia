...
func build(view) -> void:
	...
	solid(
		...
		rock,
		_body_depth,
		func(p: Vector2) -> float: return -_body_depth(p) * 0.82,
		3
	)
	...
	solid(
		...
		"SunCharm",
		gold,
		func(_p: Vector2) -> float: return 113.0,
		func(_p: Vector2) -> float: return 99.0,
		1
	)
	faceted_gem(Vector2(872, 1022), Vector2(15, 26), 118.0, 12.0, "CharmAmber", ember)
	solid(
		points([[873, 1088], [889, 1108], [869, 1150], [856, 1113]]),
		"CharmTail",
		gold,
		func(_p: Vector2) -> float: return 108.0,
		func(_p: Vector2) -> float: return 99.0,
		1
	)
	lay_on_counter(40.0)
	...
	if Geometry2D.is_point_in_polygon(pixel, body_outline):
		...
		for i in body_outline.size():
			var nearest := Geometry2D.get_closest_point_to_segment(
				pixel, body_outline[i], body_outline[(i + 1) % body_outline.size()]
			)
