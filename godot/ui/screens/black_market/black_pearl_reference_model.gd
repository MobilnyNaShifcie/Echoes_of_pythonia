...
const CRADLE := [Vector2(138,760),Vector2(183,797),Vector2(215,800),Vector2(296,770),Vector2(423,866),Vector2(551,911),Vector2(672,920),Vector2(810,894),Vector2(913,844),Vector2(1086,645),Vector2(1218,729),Vector2(1257,772),Vector2(1267,851),Vector2(1242,930),Vector2(1182,997),Vector2(1089,1056),Vector2(969,1109),Vector2(842,1144),Vector2(723,1157),Vector2(650,1152),Vector2(613,1139),Vector2(577,1147),Vector2(509,1128),Vector2(405,1098),Vector2(293,1055),Vector2(200,1009),Vector2(141,957),Vector2(113,891),Vector2(113,825)]
const LEFT_REAR_CLAW := [Vector2(179,540),Vector2(145,569),Vector2(112,611),Vector2(98,656),Vector2(100,701),Vector2(122,759),Vector2(170,812),Vector2(220,839),Vector2(251,798),Vector2(212,747),Vector2(193,691),Vector2(174,636),Vector2(170,582)]
const LEFT_FRONT_CLAW := [Vector2(352,484),Vector2(360,482),Vector2(361,490),Vector2(346,522),Vector2(325,558),Vector2(309,617),Vector2(288,672),Vector2(277,742),Vector2(281,797),Vector2(304,842),Vector2(340,869),Vector2(353,885),Vector2(317,912),Vector2(273,927),Vector2(221,935),Vector2(188,906),Vector2(181,852),Vector2(182,783),Vector2(190,710),Vector2(207,643),Vector2(239,578),Vector2(276,531),Vector2(323,495)]
const RIGHT_REAR_CLAW := [Vector2(1064,443),Vector2(1093,413),Vector2(1124,384),Vector2(1140,372),Vector2(1161,378),Vector2(1190,409),Vector2(1224,458),Vector2(1247,509),Vector2(1266,574),Vector2(1272,638),Vector2(1265,703),Vector2(1248,753),Vector2(1215,789),Vector2(1173,812),Vector2(1116,828),Vector2(1071,835),Vector2(977,807),Vector2(1001,769),Vector2(1040,721),Vector2(1072,665),Vector2(1094,608),Vector2(1107,553),Vector2(1113,504),Vector2(1104,469)]
const RIGHT_FRONT_CLAW := [Vector2(1070,789),Vector2(1127,758),Vector2(1171,721),Vector2(1205,680),Vector2(1224,639),Vector2(1238,683),Vector2(1230,746),Vector2(1212,806),Vector2(1184,862),Vector2(1145,915),Vector2(1097,964),Vector2(1036,1007),Vector2(966,1047),Vector2(886,1080),Vector2(790,1109),Vector2(704,1137),Vector2(648,1154),Vector2(619,1150),Vector2(625,1124),Vector2(642,1094),Vector2(673,1068),Vector2(706,1030),Vector2(744,1000),Vector2(797,971),Vector2(844,940),Vector2(874,908),Vector2(879,876),Vector2(877,844),Vector2(886,832),Vector2(925,824),Vector2(983,807)]
...
func build(view) -> void:
	...
	_build_ridge([Vector2(167,946),Vector2(218,996),Vector2(318,1043),Vector2(432,1080),Vector2(547,1113),Vector2(579,1128)], 5.0, 0, "LowerGoldBead")
	_build_ridge([Vector2(675,1100),Vector2(783,1065),Vector2(894,1026),Vector2(998,969),Vector2(1103,893),Vector2(1183,793)], 5.0, 2, "CarvedRimRelief")
	_build_ridge([Vector2(350,489),Vector2(309,563),Vector2(263,682),Vector2(245,784),Vector2(259,865)], 3.0, 2, "LeftClawBevel")
	...
	for row in RINGS:
		for side in SIDES:
			...
			for corner: Vector2i in [Vector2i(side,row),Vector2i(side+1,row),Vector2i(side,row+1),Vector2i(side+1,row+1)]:
				...
				var pixel := PEARL_CENTER + Vector2(cos(angle) * sin(latitude), cos(latitude)) * PEARL_RADIUS
...
func _opaque_pearl_uv(pixel: Vector2) -> Vector2:
	...
	for step in 16:
		...
		var opaque := true
		for offset: Vector2i in [Vector2i.ZERO,Vector2i(-3,0),Vector2i(3,0),Vector2i(0,-3),Vector2i(0,3)]:
...
func _build_fitting(outline: Array, node_name: String, kind: int, thickness: float) -> void:
	...
	for i in range(0, indices.size(), 3):
		_refine_face(st, polygon[indices[i]], polygon[indices[i+1]], polygon[indices[i+2]], kind, 0, true, 2)
		_refine_face(st, polygon[indices[i]], polygon[indices[i+1]], polygon[indices[i+2]], kind, -thickness, false, 2)
	...
	for i in polygon.size():
		...
		for step in 4:
			...
			for vertex in [[p,0.0],[q,0.0],[p,-thickness],[q,0.0],[q,-thickness],[p,-thickness]]:
...
func _refine_face(st: SurfaceTool, a: Vector2, b: Vector2, c: Vector2, kind: int, offset: float, front: bool, subdivisions: int) -> void:
	if subdivisions > 0:
		var ab := (a+b)*0.5
		var bc := (b+c)*0.5
		var ca := (c+a)*0.5
		for triangle in [[a,ab,ca],[ab,b,bc],[ca,bc,c],[ab,bc,ca]]:
			_refine_face(st, triangle[0], triangle[1], triangle[2], kind, offset, front, subdivisions-1)
		return
	var pixels := [a,b,c]
	var vertices := [point(a,_fitting_depth(a,kind)+offset),point(b,_fitting_depth(b,kind)+offset),point(c,_fitting_depth(c,kind)+offset)]
	...
	for i in path.size()-1:
		for step in 6:
			...
			for side in 12:
				var pixels: Array[Vector2] = []
				var vertices: Array[Vector3] = []
				for corner: Vector2i in [Vector2i(side,0),Vector2i(side+1,0),Vector2i(side,1),Vector2i(side+1,1)]:
					var angle := TAU * corner.x / 12.0
					var center := a if corner.y == 0 else b
					var pixel := center + normal * cos(angle) * width
					pixels.append(pixel)
					vertices.append(point(pixel, _fitting_depth(center,kind) + width*0.75 + sin(angle)*width))
