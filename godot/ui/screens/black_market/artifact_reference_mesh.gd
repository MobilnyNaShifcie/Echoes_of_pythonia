...
func initialize(
	view, art: Texture2D, dimensions: Vector2, axis: float, floor_y: float, silhouette_height: float
) -> void:
	host = view
	reference_art = art
	art_size = dimensions
	...
	for step in 24:
		...
		for offset: Vector2i in [
			...
			if (
				sample.x < 0
				or sample.y < 0
				or sample.x >= int(art_size.x)
				or sample.y >= int(art_size.y)
...
func solid(
	...
	for i in polygon.size():
		...
		for step in steps:
			...
			var vertices := [
				point(p, front.call(p)),
				point(q, front.call(q)),
				point(p + back_offset, rear.call(p)),
				point(q + back_offset, rear.call(q))
			]
			var uvs := [
				uv(p, center),
				uv(q, center),
				uv(p + back_offset, center + back_offset),
				uv(q + back_offset, center + back_offset)
			]
...
func _face(
	...
	emit_triangle(
		st, vertices, uvs, (Vector3.BACK if front else Vector3.FORWARD).rotated(Vector3.UP, YAW)
	)
...
func relief(
	...
	solid(
		...
		label,
		mat,
		func(p: Vector2) -> float: return float(depth.call(p)) + lift,
		func(p: Vector2) -> float: return float(depth.call(p)) + lift - thickness,
		subdivisions
	)


func oval_tube(
	center: Vector2, extent: Vector2, radius: float, depth: float, label: String, mat: Material
) -> void:
	var st := SurfaceTool.new()
	st.begin(Mesh.PRIMITIVE_TRIANGLES)
	for ring in 72:
		for side in 12:
			...
			for corner: Vector2i in [
				Vector2i(ring, side),
				Vector2i(ring + 1, side),
				Vector2i(ring, side + 1),
				Vector2i(ring + 1, side + 1)
			]:
...
func faceted_gem(
	center: Vector2, extent: Vector2, depth: float, rise: float, label: String, mat: Material
) -> void:
	var st := SurfaceTool.new()
	st.begin(Mesh.PRIMITIVE_TRIANGLES)
	...
	for i in 4:
			...
			for pixel: Vector2 in tri:
				var z := (
					...
					+ (
						rise
						if pixel == center
						else (rise * 0.75 if pixel == inner_a or pixel == inner_b else 0.0)
					)
				)
		...
		emit_triangle(
			st,
			[point(a, depth), point(center, depth - rise * 0.3), point(b, depth)],
			[uv(a, center), uv(center, center), uv(b, center)],
			Vector3.FORWARD.rotated(Vector3.UP, YAW)
		)
	finish(st, label, mat)
...
func lay_on_counter(turn_degrees: float) -> void:
	...
	var basis := (
		Basis(Vector3.UP, deg_to_rad(turn_degrees)) * Basis(Quaternion(face_normal, Vector3.UP))
	)
